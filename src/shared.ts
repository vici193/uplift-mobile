export const SECURITY_QUESTIONS = [
  {
    ...
    fil: "Ano ang pangalan ng inyong ina bago mag-asawa?",
  },
  {
    key: "first_vehicle_plate",
    en: "What was the plate number of your first vehicle?",
    fil: "Ano ang plate number ng iyong unang sasakyan?",
  },
  {
    key: "childhood_friend",
    en: "What is the name of your childhood best friend?",
    fil: "Ano ang pangalan ng iyong matalik na kaibigan noong bata ka pa?",
  },
  {
    key: "barangay_nickname",
    en: "What is your barangay's old name or nickname?",
    fil: "Ano ang lumang pangalan o palayaw ng iyong barangay?",
  },
  {
    key: "first_employer",
    en: "What was your first employer or operator's name?",
    fil: "Ano ang pangalan ng iyong unang amo o operator?",
  },
  {
    key: "first_pet_name",
    en: "What was the name of your first pet?",
    fil: "Ano ang pangalan ng iyong unang alagang hayop?",
  },
  {
    key: "parents_meeting_city",
    en: "In what city or town did your parents first meet?",
    fil: "Saang lungsod o bayan unang nagkakilala ang iyong mga magulang?",
  },
  {
    key: "first_phone_model",
    en: "What was the brand/model of your very first mobile phone?",
    fil: "Ano ang brand/model ng iyong unang cellphone?",
  },
  {
    key: "childhood_street",
    en: "What street did you grow up on (not your current address)?",
    fil: "Saang kalye ka lumaki (hindi ang kasalukuyan mong tirahan)?",
  },
  {
    key: "elementary_teacher",
    en: "What was the name of your favorite elementary school teacher?",
    fil: "Sino ang pangalan ng paborito mong guro noong elementarya?",
  },
];

export function securityQuestionLabel(key: string, en: boolean) {
  if (!key) return "";
...
export function toProperCaseKeepAcronyms(str: string) {
  if (!str) return str;
  return str
    .trim()
    .split(/(\s+)/)
...
export function formatMobileDisplay(v: string) {
...
export function formatEwalletNumber(v: string) {
  const clean = (v || "").replace(/[^0-9]/g, "").slice(0, 11);
  const p1 = clean.slice(0, 4),
    p2 = clean.slice(4, 7),
    p3 = clean.slice(7, 11);
...
export function formatPlateNumber(v: string) {
...
export function formatLicenseNumber(v: string) {
  const clean = (v || "")
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, "")
    .slice(0, 11);
  const seg1 = clean.slice(0, 3),
    seg2 = clean.slice(3, 5),
    seg3 = clean.slice(5, 11);
  return [seg1, seg2, seg3].filter(Boolean).join("-");
}

export function formatCaseNumber(v: string) {
  const clean = (v || "").replace(/[^0-9]/g, "").slice(0, 8);
  const seg1 = clean.slice(0, 4),
    seg2 = clean.slice(4, 8);
  return [seg1, seg2].filter(Boolean).join("-");
}

export const DENOMINATION_DL_CODE_HINT = {
  MPUJ: {
    en: "Should show DL Code B1 (9+ passenger seats, up to 5,000kg GVW).",
    fil: "Dapat may DL Code B1 (9+ upuan, hanggang 5,000kg GVW).",
  },
  TPUJ: {
    en: "Should show DL Code B1 (9+ passenger seats, up to 5,000kg GVW).",
    fil: "Dapat may DL Code B1 (9+ upuan, hanggang 5,000kg GVW).",
  ...
  MPUB: {
    en: "Should show DL Code D (buses, over 5,000kg GVW, 9+ passenger seats).",
    fil: "Dapat may DL Code D (bus, higit sa 5,000kg GVW, 9+ upuan).",
  },
  PUB: {
    en: "Should show DL Code D (buses, over 5,000kg GVW, 9+ passenger seats).",
    fil: "Dapat may DL Code D (bus, higit sa 5,000kg GVW, 9+ upuan).",
  ...
  "Mini-Bus": {
    en: "Should show DL Code B1 or D, depending on your unit's gross vehicle weight.",
    fil: "Dapat may DL Code B1 o D, depende sa gross vehicle weight ng iyong sasakyan.",
  },
  "School Transport": {
    en: "Should show DL Code B1 or D, depending on your unit's gross vehicle weight.",
    fil: "Dapat may DL Code B1 o D, depende sa gross vehicle weight ng iyong sasakyan.",
  ...
  Taxi: {
    en: "Should show DL Code B (up to 8 passenger seats) or B1 for larger units.",
    fil: "Dapat may DL Code B (hanggang 8 upuan) o B1 para sa mas malaking sasakyan.",
  },
};

export function dlCodeHint(denomination: string, en: boolean) {
  const found = DENOMINATION_DL_CODE_HINT[denomination as keyof typeof DENOMINATION_DL_CODE_HINT];
...
export const DENOMINATION_DL_CODE_SHORT = {
  ...
  PUB: "D",
  "Mini-Bus": "B1 or D",
  "School Transport": "B1 or D",
  Taxi: "B or B1",
};

export function licenseNumberPlaceholder(denomination: string) {
  const code = DENOMINATION_DL_CODE_SHORT[denomination as keyof typeof DENOMINATION_DL_CODE_SHORT];
