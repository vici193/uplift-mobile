...
function AdminProfile() {
  ...
  const { en, handleLogout } = useSession();

  const rows = [
    { icon: ShieldCheck, label: en ? "Roles & permissions" : "Mga Tungkulin at Pahintulot", to: "/admin-roles" },
    { icon: KeyRound, label: en ? "Change password" : "Palitan ang password", to: "/password" },
    { icon: Settings, label: en ? "Settings" : "Mga Pagsasaayos", to: "/settings" },
  ];
  ...
  return (
    <MobileShell bottomNav={<AdminBottomNav />}>
      <TopBar title={en ? "Admin Profile" : "Admin Profile"} />
        ...
        <div className="relative overflow-hidden rounded-[32px] bg-[#1b2b4b] p-6 text-white shadow-xl transition-all duration-300 hover:bg-[#253960]">
          ...
          <div className="relative z-20 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-white/70 font-medium">{en ? "DOTr Administrator" : "Tagapangasiwa ng DOTr"}</p>
              <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-emerald-500/20 px-2.5 py-1 text-[10px] font-bold text-emerald-400 border border-emerald-500/30">
                <CheckCircle2 className="h-3 w-3" /> {en ? "Verified" : "Kumpirmado"}
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-2 border-t border-white/10 pt-5 text-center">
            <div>
              ...
              <p className="text-[9px] font-bold uppercase text-white/50 tracking-wider">{en ? "Total Users" : "Kabuuang Gumagamit"}</p>
            </div>
            <div>
              <p className="text-[16px] font-extrabold">48</p>
              <p className="text-[9px] font-bold uppercase text-white/50 tracking-wider">{en ? "Active" : "Aktibo"}</p>
            </div>
            <div>
              <p className="text-[16px] font-extrabold">5</p>
              <p className="text-[9px] font-bold uppercase text-white/50 tracking-wider">{en ? "Reports" : "Mga Ulat"}</p>
            </div>
          </div>
        </div>
      </div>
