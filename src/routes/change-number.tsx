...
import numberSticker from "@/assets/number.png";
...
const steps = [
  ...
  { id: 4, label: "Update", icon: ArrowRightLeft },
];
  ...
  return (
    <MobileShell className="relative flex min-h-screen flex-col bg-[#1b2b4b] font-sans text-[#ffffff]">
      ...
      <div className="sticky top-0 z-30 flex items-center gap-4 px-6 py-6 backdrop-blur-md">
        ...
        <h1 className="text-[16px] font-bold uppercase tracking-widest">Change Number</h1>
      </div>
      ...
      <div className="flex-1 px-6 pt-4 pb-12 z-10 flex flex-col justify-center">
        ...
        <div className="p-8 rounded-3xl bg-[#1b2b4b]/50 border border-[#ffffff]/10 shadow-xl relative backdrop-blur-sm min-h-[320px] flex flex-col justify-center">
           <img src={numberSticker} alt="Number" className="absolute -top-12 -right-2 w-40 h-40 z-20 opacity-90" />
           
          {step === 1 && (
            <div className="space-y-4">
              <h2 className="text-[18px] font-bold text-[#ffffff]">Verify Identity</h2>
              <p className="text-[12px] text-[#8c8b88] leading-relaxed">Confirm your current details.</p>
              <Field label="Current Mobile Number" placeholder="09XX XXX XXXX" icon={Phone} type="tel" />
              <Field label="Security Password" placeholder="••••••••" icon={Lock} type="password" />
            </div>
          )}
          {step === 2 && (
            <div className="space-y-4">
          ...
          {step === 4 && (
            <div className="space-y-4">
              <h2 className="text-[18px] font-bold text-[#ffffff]">New Number</h2>
              <p className="text-[12px] text-[#8c8b88] leading-relaxed">Set your new primary contact number.</p>
              <Field label="New Mobile Number" placeholder="09XX XXX XXXX" icon={Phone} type="tel" />
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-3 p-4 mt-8 rounded-3xl bg-[#ffffff]/5 border border-[#ffffff]/5">
            <div className="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-[#8c8b88]">Your information is encrypted and secure.</p>
                </div>
            </div>
        </div>
        ...
        <button 
          type="button" 
          onClick={() => step < 4 ? setStep(s => s + 1) : navigate({ to: "/" })} 
